...
uint256 hashGenesisBlock("0x0000061367bd6ec3b1d2c28117d41b80f2589720f9a7c3fa06689a0f0c0945f6"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bit Signed Message:\n";
...
{
    // Bit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 0; 
    if(nHeight >= 3456) {
        if((nHeight >= 7000 && dDiff > 75) || nHeight >= 14000) { // GPU/ASIC difficulty calc
            // 7777777/(((x+8000)/10)^3)
            nSubsidy = (7777777.0 / (pow((dDiff+8000.0)/10.0,3.0)));
            if (nSubsidy > 25) nSubsidy = 25;
            if (nSubsidy < 5) nSubsidy = 5;
        } else { // CPU mining calc
            nSubsidy = (55555.0 / (pow((dDiff+71.0)/7.0,3.0)));
            if (nSubsidy > 25) nSubsidy = 25;
            if (nSubsidy < 15) nSubsidy = 15;
        }
    } else {
        nSubsidy = (4444.0 / (pow((dDiff+1.0),3.0)));
        if (nSubsidy > 25) nSubsidy = 25;
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Bit: 1 day
static const int64 nTargetSpacing = 3.5 * 60; // Bit: 3.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // Bit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "BIT-The New World Order";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407225726;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1147643;
        ...
        assert(block.hashMerkleRoot == uint256("0x92e639f1aab4dc51aebb3fb9bbcaa9e33e214b2f43942f32f4ecfd3459f73596"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitMiner(CWallet *pwallet)
{
    printf("BitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bit-miner");
    ...
    try { loop {
        ...
        printf("Running BitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - BIT **********
    Copyright 2014, Bit Developers - ( evan@bit.io )
*/
